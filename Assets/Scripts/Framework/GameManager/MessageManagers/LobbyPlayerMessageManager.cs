...
namespace Framework.MessageManagers
{
    ...
    public class LobbyPlayerMessageManager : IMessageBehaviour
    {
        ...
        public async FTask<StateSyncResponse> SendStateSyncRequest(StateSyncData syncData)
        {
            ...
            syncData.playerId = GameManager.Core.World.GetExitsDataManager<UserDataManager>().UserData.AccountId;
        ...
        public class OtherPlayerLoginMessageHandler : Message<OtherPlayerLoginMessage>
        {
            protected override async FTask Run(Session session, OtherPlayerLoginMessage message)
            {
                ...
                player.GetComponent<LobbyPlayerName>().Init(message.playerData.PlayerName , GameManager.Core.World.GetExitsLogicManager<TP_CameraLogicManager>().cameraControl.transform);
                ...
                //缓存其他玩家
                GameManager.Core.World.GetExitsDataManager<LobbyPlayerDataManager>().AddOtherPlayer(message.playerData.playerId, playerScript);
        ...
        public class OtherPlayerLogoutMessageHandler : Message<OtherPlayerLogoutMessage>
        {
            ...
            {
                //移除对应的玩家
                var otherPlayer = GameManager.Core.World.GetExitsDataManager<LobbyPlayerDataManager>().GetOtherPlayer(message.playerId);
                ...
                Object.Destroy(go);
                GameManager.Core.World.GetExitsDataManager<LobbyPlayerDataManager>().RemoveOtherPlayer(message.playerId);
        ...
        public class OtherPlayerStateSyncMessageHandler : Message<OtherPlayerStateSyncMessage>
        {
            ...
            {
                var otherPlayer = GameManager.Core.World.GetExitsDataManager<LobbyPlayerDataManager>().GetOtherPlayer(message.roleData.playerId);
                if (otherPlayer == null)
                {
